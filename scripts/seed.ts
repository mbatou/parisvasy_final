...
async function upsertByUnique<T extends Record<string, unknown>>(
  table: string,
  data: T,
  uniqueColumn: string
) {
  ...
  const { data: existing } = await supabase
    .from(table)
    .select("*")
    .eq(uniqueColumn, data[uniqueColumn] as string)
    .single();
...
async function main() {
  ...
  const roomsData = [
    ...
    },
    {
      ...
      size: 32,
      maxGuests: 3,
      amenities: ["wifi", "minibar", "air_conditioning", "safe", "balcony", "bathtub", "nespresso"],
      images: [],
      pricePerNight: 189,
      totalRooms: 5,
      isActive: true,
    },
    {
      ...
      size: 45,
      maxGuests: 2,
      amenities: ["wifi", "minibar", "air_conditioning", "safe", "eiffel_view", "balcony", "bathtub", "rain_shower", "nespresso", "living_area"],
      images: [],
      pricePerNight: 289,
      totalRooms: 3,
      isActive: true,
  ...
  const rooms = await Promise.all(
    roomsData.map((r) => upsertByUnique("Room", r, "id"))
  );
  ...
  const experiencesData = [
    {
      ...
      duration: "2h",
      maxGroup: 12,
      inclusions: ["Champagne toast", "Live acoustic music", "Expert commentary", "Blankets provided"],
      images: [],
      isFlash: false,
      isActive: true,
    },
    {
      ...
      duration: "3h",
      maxGroup: 8,
      inclusions: ["5 wine tastings", "Artisan cheese selection", "Expert sommelier guide", "Montmartre walking tour"],
      images: [],
      isFlash: false,
      isActive: true,
    },
    {
      ...
      duration: "2.5h",
      maxGroup: 6,
      inclusions: ["Skip-the-line entry", "Expert art historian guide", "Personal audio headset", "Museum map & guide book"],
      images: [],
      isFlash: false,
      isActive: true,
    },
    {
      ...
      duration: "1.5h",
      maxGroup: 4,
      inclusions: ["Traditional steam bath", "30-minute full body massage", "Refreshing mint tea", "Organic skincare products"],
      images: [],
      isFlash: false,
      isActive: true,
    },
    {
      ...
      duration: "2h",
      maxGroup: 10,
      inclusions: ["Gourmet picnic basket", "Cozy blankets & cushions", "Bottle of champagne", "Macarons from Laduree"],
      images: [],
      isFlash: true,
      flashStart: new Date("2026-03-15").toISOString(),
      flashEnd: new Date("2026-04-30").toISOString(),
      isActive: true,
    },
    {
      hotelId: hotel.id,
      ...
      duration: "3h",
      maxGroup: 8,
      inclusions: ["Professional chef instruction", "All ingredients provided", "Recipe book to take home", "Glass of wine during class"],
      images: [],
      isFlash: false,
      isActive: true,
    },
  ...
  const experiences = await Promise.all(
    experiencesData.map((e) => upsertByUnique("Experience", e, "slug"))
  );
  ...
  for (const seedUser of seedUsers) {
    ...
    if (userId) {
      const guest = await upsertByUnique("Guest", {
        authUserId: userId,
        email,
  ...
  const sampleGuest = await upsertByUnique("Guest", {
  ...
  const sampleGuest2 = await upsertByUnique("Guest", {
  ...
  await upsertByUnique("Booking", {
    ...
    checkOut: new Date(tomorrow.getTime() + 3 * 24 * 60 * 60 * 1000).toISOString(),
    nights: 3,
    guestCount: 2,
    roomTotal: 867,
    status: "pending",
